...
function Inbox({conversations,selected,setSelected,updateConversation}:{conversations:Conversation[],selected:Conversation,setSelected:(id:string)=>void,updateConversation:(c:Conversation)=>void}){
  ...
  async function draft(){setDrafting(true);const r=await fetch('/api/agent/draft',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({stage:selected.stage,painPoint:selected.painPoint,objection:selected.objection})});const j:any=await r.json();setInput(j.draft||'');setDrafting(false);}
...
function Area({label,value,onChange}:{label:string,value:string,onChange:(v:string)=>void}){return <label><span>{label}</span><textarea value={value||''} onChange={e=>onChange(e.target.value)}/></label>}
